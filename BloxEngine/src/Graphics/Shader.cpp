...
void Shader::generate(const char* vertexShaderPath, const char* fragmentShaderPath)
{
	int success;
	char infoLog[512];
	unsigned int vertexShader = CompileShader("BloxEngine/assets/vertex_core.glsl", GL_VERTEX_SHADER);
	unsigned int fragmentShader = CompileShader("BloxEngine/assets/fragment_core.glsl", GL_FRAGMENT_SHADER);
